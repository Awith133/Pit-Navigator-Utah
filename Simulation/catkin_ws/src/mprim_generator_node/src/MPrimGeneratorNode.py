...
system_path = rospy.get_param('/system_name')
...
def newMotionPrimitives(outfilename):
    ...
    for angleind in range(numberofangles):
        ...
        for primind in range(numberofprimsperangle):
            ...
            #current angle
            currentangle = (angleind-1)*2*math.pi/numberofangles 
            ...
            if (UNICYCLE_MPRIM_16DEGS == 1):
                ...
                endpt = [endpose_c[0]*resolution, endpose_c[1]*resolution, ((angleind - 1 + baseendpose_c[2])% numberofangles )*2*math.pi/numberofangles]
                intermcells_m = np.zeros((numofsamples,3)) 
                if ((endx_c == 0 and endy_c == 0) or baseendpose_c[2] == 0): #turn in place or move forward            
                    for iind in range(0, numofsamples):
                        intermcells_m[iind,:] = [startpt[0] + (endpt[0] - startpt[0])*(iind)/(numofsamples), 
                                                startpt[1] + (endpt[1] - startpt[1])*(iind)/(numofsamples), 
                                                0] 
                        rotation_angle = (baseendpose_c[2] ) * (2*math.pi/numberofangles) 
                        intermcells_m[iind,2] = (startpt[2] + (rotation_angle)*(iind)/(numofsamples))% 2*math.pi 
                    ...
                    S = np.linalg.pinv(R)*np.array([[endpt[0] - startpt[0]] , [endpt[1] - startpt[1]]])
                    l = S[0]  
                    tvoverrv = S[1] 
                    ...
                    for iind in range(numofsamples):                                       
                        ...
                        if(abs(dt*tv) < abs(l)):
                            intermcells_m[iind,:] = [startpt[0] + dt*tv*math.cos(startpt[2]),
                                                     startpt[1] + dt*tv*math.sin(startpt[2]),
                                                     startpt[2]] 
                        else:
                            dtheta = rv*(dt - l/tv) + startpt[2] 
                            intermcells_m[iind,:] = [startpt[0] + l*math.cos(startpt[2]) + tvoverrv*(math.sin(dtheta) - math.sin(startpt[2])),
                                                     startpt[1] + l*math.sin(startpt[2]) - tvoverrv*(math.cos(dtheta) - math.cos(startpt[2])),
                                                     dtheta] 
                         
                      
                    #correct
                    errorxy = [endpt[0] - intermcells_m[numofsamples,0] ,
                               endpt[1] - intermcells_m[numofsamples,1]] 
                    print( 'l=%f errx=%f erry=%f\n'% (l, errorxy[0], errorxy[1])) 
                    interpfactor = np.linspace(0,1,numofsamples-1)
