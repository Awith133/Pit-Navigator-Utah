...
public:
  ...
  void addNeighborCellToOpenList(multiset<cells> & OPL,int currentCell, int neighborCell, int goalCell, float g_score[]);
  vector <int> findFreeNeighborCell (int CellID);

  vector <int> sunFilterNeighborCells (vector <int>neighborCells, int CellID);
  vector <int> sunTacking (vector <int>neighborCells, int CellID);
  double solarCost (int CellID1, int CellID2, int goalCell);
  void solarCostUpdate ();
  void solarCostReset();
  double obstacleCost(int CellID1);
